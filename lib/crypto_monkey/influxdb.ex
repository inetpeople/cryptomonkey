defmodule CryptoMonkey.InfluxDB do
  use Instream.Connection, otp_app: :crypto_monkey
  require Logger

  # @db "crypto_monkey"
  @db Application.get_env(:crypto_monkey, :influx_db_name)
  ...
  def create_retention_policies do
    ...
    Instream.Admin.RetentionPolicy.alter(
      "realtime",
      @db,
      "DURATION 1800d REPLICATION 1 DEFAULT"
    )
    # default
    |> execute()
  ...
  def write_points(points, retention_policy \\ "realtime") do
    case %{database: @db, points: points}
         |> write(retention_policy: retention_policy, database: @db) do
      :ok = resp ->
